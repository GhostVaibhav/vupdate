...
int download_progress_callback(void* clientp, curl_off_t dltotal,
                               ...
                               curl_off_t ulnow)
{
  if((dltotal == 0 && progress == -1) || (dltotal && progress != static_cast<float>(dlnow) / static_cast<float>(dltotal) * 100)) {
    ...
    {
      float percentage = static_cast<float>(dlnow) / static_cast<float>(dltotal) * 100;
...
void getFile(std::string server, std::string file, std::shared_ptr<spdlog::logger>& logger) {
  indicators::ProgressBar progress_bar{
      indicators::option::BarWidth{30}, indicators::option::Start{" ["},
      indicators::option::Fill{"="}, indicators::option::Lead{">"},
      indicators::option::End{"]"}, indicators::option::PostfixText{" | " + file.substr(0, 64)},
      indicators::option::ShowPercentage{true}, indicators::option::MaxPostfixTextLen{15}
  };
