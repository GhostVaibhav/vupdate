...
void consolid(std::string file, std::shared_ptr<spdlog::logger>& logger, nlohmann::json& localFilelistJson) {
    ...
    if (lastIndex < file.size())
        directories.append(file.substr(0, lastIndex));
    logger->info("Directories: {}", directories);
    if (lastIndex < file.size()) {
        directories = file.substr(0, lastIndex);
        if(!std::filesystem::exists(directories)) if(std::filesystem::create_directories(directories))  logger->info("Created directory: {}", directories);
        if(lastIndex != 1) {
            std::ofstream localFilelist("localFilelist.json");
            localFilelistJson["files"][file] = sha256_file((char*) rawFile.c_str());
            std::filesystem::copy_file("./" + rawFile, directories + "/" + rawFile, std::filesystem::copy_options::overwrite_existing);
